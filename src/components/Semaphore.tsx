    ...
    <div className="w-full flex flex-row items-start justify-center gap-2 sm:gap-4 md:gap-6 p-4 sm:p-8 bg-neutral-900/95 border-b-4 border-neutral-800 shadow-[0_20px_60px_rgba(0,0,0,0.8)] relative overflow-hidden">
      {[...Array(5)].map((_, i) => (
        <div key={i} className="flex flex-col items-center flex-1 max-w-[140px]">
          {/* Hanging Connector Rod - Anchors the gantry to the roof */}
          <div className="h-6 sm:h-12 w-2 sm:w-5 bg-neutral-800 rounded-t shadow-[inset_0_2px_10px_rgba(0,0,0,0.8)] flex-shrink-0" />
          {/* Light Pod Wrapper */}
          <div className="w-full aspect-[4/9] min-h-[120px] sm:min-h-[160px] bg-neutral-950 rounded-sm sm:rounded-lg border-2 border-neutral-800 flex flex-col items-center justify-evenly py-1 shadow-[inset_0_0_25px_rgba(0,0,0,1)] relative overflow-hidden">
            {/* Top Light Unit */}
            <div className="w-[75%] relative z-10">
              <div
                className={cn(
                  "w-full aspect-square rounded-full transition-none", // CRITICAL: No transitions for timing accuracy
                  i < lightsActive
                    ? "bg-red-600 glow-red border-red-400 border-2"
                    : "bg-neutral-900 border-neutral-800 border-2 shadow-none"
                )}
              >
                {/* Internal Lens Texture */}
                <div className="absolute inset-0 rounded-full bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.1)_0%,transparent_70%)] opacity-50" />
              </div>
            </div>
            {/* Bottom Light Unit */}
            <div className="w-[75%] relative z-10">
            ...
            {/* Micro-mesh scanline texture inside pod */}
      ...
      {/* Decorative high-tech structural grid overlay */}
      ...
      {/* Subtle lighting bleed from active pods onto the gantry bar */}
      <div className={cn(
        "absolute inset-0 transition-opacity duration-300 pointer-events-none",
        lightsActive > 0 ? "opacity-10 bg-red-900/20" : "opacity-0"
      )} />
    </div>
  );
}